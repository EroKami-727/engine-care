...
# --- CONFIG FOR REGIME A (Simple) ---
# FD001 and FD003 only
DATA_FILES = ['../data/train_FD001.txt', '../data/train_FD003.txt']
WORKER_ASSET_PATH = '../workers/jet-engine/model_assets'
MODEL_SAVE_PATH = os.path.join(WORKER_ASSET_PATH, 'model_regime_A.keras')
SCALER_SAVE_PATH = os.path.join(WORKER_ASSET_PATH, 'scaler_regime_A.pkl')
...
def load_data():
    print("🚀 [REGIME A] Loading Stable Datasets (FD001 + FD003)...")
    all_dfs = []
...
def build_model(input_shape):
    ...
    # Transformer Block
    x = layers.MultiHeadAttention(key_dim=128, num_heads=4, dropout=0.1)(inputs, inputs)
...
def train():
    ...
    # MinMaxScaler is best for stable regimes
    scaler = MinMaxScaler()
    ...
    model.fit(X, y, epochs=200, batch_size=512, validation_split=0.15, callbacks=[early_stop, checkpoint], verbose=1)
    print("✅ Regime A Model Saved.")
